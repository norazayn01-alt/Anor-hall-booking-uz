...
class AnimationController {
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      ...
      this.ctx.fillStyle = "rgba(4, 120, 87, 0.12)";
      this.ctx.lineWidth = sw;
...
export function SpiralAnimation() {
  ...
  return (
    <div className="relative w-full h-full bg-white overflow-hidden select-none pointer-events-none" style={{ backgroundColor: "#ffffff" }}>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full block opacity-40" style={{ backgroundColor: "#ffffff" }} />
    </div>
  );
}
